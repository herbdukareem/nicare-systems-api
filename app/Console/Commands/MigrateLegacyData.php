...
class MigrateLegacyData extends Command
{
    ...
    public function handle()
    {
        ...
        // DB::transaction(function () {
            ...
            // 3. Migrate funding types - FIXED TYPO IN TABLE NAME
           ...
           // 5. Migrate providers into facilities
            ...
            foreach ($legacyPins as $pin) {
                ...
                $sector_id = $this->enrollee_type_array[strtolower($pin->category)] ?? null;
                
                if (!$sector_id) {
                    $this->warn("Sector not found for pin category: {$pin->category}");
                    continue;
                }
                ...
                $newPremium = Premium::updateOrCreate(
                    ...
                    [
                        ...
                        'lga_id'        => $this->lga_array[$pin->lga] ?? null,
                        'ward_id'       => $this->ward_array[$pin->ward] ?? null,
        ...
        DB::connection('legacy')
            ...
            ->chunkById(100, function ($legacyEnrollees) use ($sector) {
                foreach ($legacyEnrollees as $enrollee) {
                    ...
                    $funding = $enrollee->funding ? 
                        ($this->funding_array[$enrollee->funding] ?? $this->funding_array['premium'] ?? null) : 
                        ($this->funding_array['premium'] ?? null);
                    ...
                    $relationship_to_principal = match($enrollee->enrolee_category) {
                        ...
                        default => RelationshipToPrincipal::PRINCIPAL,
                    };
                    ...
                    $vulnerable_group_id = $this->vulnerable_groups_arr[strtolower($enrollee->vulnerability_status)] ?? 
                        $this->vulnerable_groups_arr['none'] ?? null;
                    ...
                    $newEnrollee = Enrollee::updateOrCreate(
                        ...
                        [   
                            ...
                            'benefit_package_id' => $enrollee->mode_of_enrolment == 'huwe' ? 
                                $this->BMPHSPackage->id : $this->standardPackage->id,
                            ...
                            'facility_id'     => $this->facility_array[$enrollee->provider_id] ?? null,
                            'lga_id'          => $this->lga_array[$enrollee->lga] ?? null,
                            'ward_id'         => $this->ward_array[$enrollee->ward] ?? null,
        ...
        foreach ($paymentInvoices as $paymentInvoice) {
            $newPaymentInvoice = Invoice::updateOrCreate(
                ...
                [
                    ...
                    'payable_id' => $this->premium_type_array[strtolower($paymentInvoice->pin_type)] ?? null,
                    'payment_catgory_id' => $paymentInvoice->request_type == 'new' ? 
                        $subscriptionType->id : $reSubscriptionType->id,
                    'metadata' => [
    ...
    protected function updatePremiumUsedby()
    {
        ...
        foreach ($enrollees as $enrollee) {
            ...
            if ($premium_id) {
                // FIXED: Added table alias 'pin' for the update
                $affected = DB::connection('legacy')
                    ->table('tbl_pin_inven as pin')
                    ->where('pin.id', $premium_id)
                    ->update(['pin.enrollee_id' => $enrollee_id]);
            }
        }

        // Uncomment if you need to process formal enrollees
        /*
        ...
        foreach ($enrollees as $enrollee) {
            ...
            if ($premium_id) {
                DB::connection('legacy')
                    ->table('tbl_pin_inven_formal as pin')
                    ->where('pin.id', $premium_id)
                    ->update(['pin.enrollee_id' => $enrollee_id]);
            }
        }
    ...
    protected function createEnrollmentPhases()
    {
        ...
        foreach ($enrollees_phases as $enrollee_phase) {
            ...
            if (!isset($this->enrollment_phase_array[$enrollee_phase->tracking])) {
                $this->enrollment_phase_array[$enrollee_phase->tracking] = [];
            }
    ...
    protected function getImage($id, $sector)
    {
        ...
        try {
            ...
            if ($response->failed() || $response->body() === '') {
                $this->warn("Failed to fetch image from {$remoteUrl}");
                return null;
            }
            ...
            $this->warn("Error fetching image for ID {$id}: " . $e->getMessage());
            return null;
        }
    }
}