    ...
    public function up(): void
    {
        Schema::table('referrals', function (Blueprint $table) {
            ...
            $table->enum('service_selection_type', ['bundle', 'direct'])
                  ...
                  ->comment('Type of service selection: bundle (ServiceBundle), direct (CaseRecord), or null (none)');
            ...
            // Link to case record if direct service is selected
            $table->foreignId('case_record_id')
                  ->nullable()
                  ->after('service_bundle_id')
                  ->constrained('case_records')
                  ->onDelete('restrict')
                  ->comment('Selected direct service for single-service referrals');
        });
    }
