    ...
    <div class="tw-space-y-6">
      ...
      <div class="tw-bg-white tw-rounded-lg tw-shadow-sm">
        ...
        >
          ...
          <template #item.drug_unit_price="{ item }">
            ₦{{ Number(item.drug_unit_price).toLocaleString() }}
          </template>

          <template #item.actions="{ item }">
            <div class="tw-flex tw-space-x-2">
              <v-btn icon size="small" variant="text" @click="viewDrug(item)">
                <v-icon>mdi-eye</v-icon>
              </v-btn>
              <v-btn icon size="small" variant="text" @click="editDrug(item)">
                <v-icon>mdi-pencil</v-icon>
              </v-btn>
              <v-btn icon size="small" variant="text" color="red" @click="deleteDrug(item)">
        ...
        <v-card-text>
          <v-form ref="drugForm" v-model="formValid">
            <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-4">
              <v-text-field v-model="drugForm.nicare_code" label="NiCare Code" :rules="[rules.required]" variant="outlined" required />
              <v-text-field v-model="drugForm.drug_name" label="Drug Name" :rules="[rules.required]" variant="outlined" required />
              <v-select v-model="drugForm.drug_dosage_form" :items="dosageFormOptions" label="Dosage Form" :rules="[rules.required]" variant="outlined" required clearable />
              <v-select v-model="drugForm.drug_strength" :items="strengthOptions" label="Strength" variant="outlined" clearable />
              <v-select v-model="drugForm.drug_presentation" :items="presentationOptions" label="Presentation" :rules="[rules.required]" variant="outlined" required clearable />
              <v-text-field v-model="drugForm.drug_unit_price" label="Unit Price" type="number" step="0.01" :rules="[rules.required, rules.positive]" variant="outlined" required />
            </div>
            <v-switch v-model="drugForm.status" label="Active" color="primary" />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="closeDialog">Cancel</v-btn>
          <v-btn color="primary" :loading="saving" :disabled="!formValid" @click="saveDrug">
    ...
    <v-dialog v-model="showImportDialog" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="importing" :disabled="!importFile" @click="importDrugs">Import</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </AdminLayout>
...
// IMPORTANT: single File (not array)
...
const pagination = ref({ page: 1, itemsPerPage: 15, totalItems: 0 })
...
const saveDrug = async () => {
  try {
    ...
    if (editingDrug.value) {
      ...
      if (response.data.success) success('Drug updated successfully')
      else return error(response.data.message || 'Failed to update drug')
    } else {
      const response = await drugAPI.create(drugForm.value)
      if (response.data.success) success('Drug created successfully')
      else return error(response.data.message || 'Failed to create drug')
    }
    closeDialog()
    loadDrugs()
    loadStatistics()
    ...
    else if (err.response?.data?.errors) error(Object.values(err.response.data.errors).flat().join(', '))
    else error('Failed to save drug')
  } finally {
    saving.value = false
  }
}

const viewDrug = (drug) => router.push(`/drugs/${drug.id}`)
...
const deleteDrug = async (drug) => {
  if (!confirm(`Are you sure you want to delete ${drug.drug_name}?`)) return
  try {
...
// Normalize v-file-input to a single File
...
const importDrugs = async () => {
  ...
  try {
    ...
    // append the REAL binary file + filename
    fd.append('file', importFile.value, importFile.value.name)
    fd.append('import_type', 'drugs')
    fd.append('timestamp', new Date().toISOString())

    const response = await drugAPI.import(fd)
    if (response.data?.success) {
      const { imported_count = 0, errors = [] } = response.data.data || {}
      if (errors.length) {
        console.warn('Import errors:', errors)
        error(`Imported with ${errors.length} errors. Check logs.`)
      } else {
    ...
    else if (err.code === 'NETWORK_ERROR') error('Network error. Please check your connection.')
    else if (err.request) error('No response from server. Please try again.')
    else error('Failed to import drugs. Please try again.')
  } finally {
...
const exportDrugs = async () => {
  try {
    const params = { search: searchQuery.value, status: selectedStatus.value }
