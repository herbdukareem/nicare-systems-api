      ...
      <v-row>
        <v-col cols="12">
          <div class="d-flex justify-space-between align-center mb-4">
            <div>
              <h2 class="text-h4 font-weight-bold">Service Categories</h2>
              <p class="text-subtitle-1 text-grey-darken-1">Manage healthcare service categories</p>
            </div>
            <v-btn
              color="primary"
      ...
      <v-dialog v-model="dialog" max-width="600px" persistent>
        <v-card>
          <v-card-title class="text-h5">
            {{ isEditing ? 'Edit' : 'Create' }} Service Category
          </v-card-title>
        ...
        <v-card v-if="selectedCategory">
          ...
          <v-card-text>
            <v-row>
              ...
              <!-- Show associated services count if available -->
              <v-col cols="12" md="6" v-if="selectedCategory.services">
                <v-list-item>
                  <v-list-item-title class="text-subtitle-2">Associated Services</v-list-item-title>
                  <v-list-item-subtitle>{{ selectedCategory.services.length }} services</v-list-item-subtitle>
                </v-list-item>
              </v-col>

              <!-- Show associated tariff items count if available -->
              <v-col cols="12" md="6" v-if="selectedCategory.tariff_items">
                <v-list-item>
                  <v-list-item-title class="text-subtitle-2">Tariff Items</v-list-item-title>
                  <v-list-item-subtitle>{{ selectedCategory.tariff_items.length }} items</v-list-item-subtitle>
                </v-list-item>
              </v-col>
            </v-row>
          </v-card-text>
...
import { serviceCategoryAPI } from '../../utils/api'
...
const saveCategory = async () => {
  ...
  try {
    let response
    if (isEditing.value) {
      response = await serviceCategoryAPI.update(selectedCategory.value.id, formData)
    } else {
      response = await serviceCategoryAPI.create(formData)
    }
...
const toggleStatus = async (category) => {
  try {
    const response = await serviceCategoryAPI.toggleStatus(category.id)
    if (response.data.success) {
...
const confirmDelete = async () => {
  ...
  deleting.value = true
  try {
    const response = await serviceCategoryAPI.delete(categoryToDelete.value.id)
    if (response.data.success) {
