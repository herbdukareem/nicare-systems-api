        ...
        <div class="tw-space-y-4">
          ...
          <section class="enroll__panel">
            ...
            <v-form @submit.prevent="submitApplication">
              ...
              <AppAlert
                v-if="requiresCheckoutFlow && form.enrollment_method !== 'bank_transfer'"
                class="tw-mt-3"
                tone="info"
...
const submitApplication = async () => {
  ...
  try {
    ...
    successSummary.value = responseData?.requires_payment && responseData?.enrollment_method === 'bank_transfer'
      ? `Application ${enrolleeId} submitted. Transfer the exact total amount${totalAmount ? ` of NGN ${Number(totalAmount).toLocaleString()}` : ''} using reference ${paymentRef}. Approval continues after payment confirmation${publicNinVerificationEnabled.value ? ' and live NIN verification' : ''}.`
      : responseData?.requires_payment
