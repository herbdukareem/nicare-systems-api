    ...
    <div class="tw-space-y-5">
      ...
      <div class="tw-grid tw-gap-3 md:tw-grid-cols-3 xl:tw-grid-cols-6">
        <div v-for="stat in stats" :key="stat.label" class="tw-rounded-lg tw-border tw-border-slate-200 tw-bg-white tw-p-4">
          <p class="tw-text-xs tw-font-medium tw-uppercase tw-text-slate-500">{{ stat.label }}</p>
        ...
        >
          ...
          <template #item.actions="{ item }">
            <div class="tw-flex tw-justify-end tw-gap-1">
              <v-btn icon size="small" variant="text" @click="openDetails(item)"><v-icon size="18">mdi-eye-outline</v-icon></v-btn>
              <v-btn icon size="small" variant="text" :disabled="!canEdit" @click="openEdit(item)"><v-icon size="18">mdi-pencil</v-icon></v-btn>
              <v-btn icon size="small" variant="text" color="primary" @click="printIdCard(item)"><v-icon size="18">mdi-card-account-details-outline</v-icon></v-btn>
            </div>
          </template>
        </v-data-table>
      </v-card>
      ...
      <v-navigation-drawer v-model="detailDrawer" location="right" temporary width="560">
        <div v-if="selected" class="tw-space-y-4 tw-p-5">
          ...
          <div class="tw-flex tw-gap-2">
            <v-btn size="small" color="primary" variant="outlined" :disabled="!canEdit" @click="openEdit(selected)">Edit</v-btn>
            <v-btn size="small" color="primary" @click="printIdCard(selected)">Print ID Card</v-btn>
          </div>

          <section v-for="group in detailGroups" :key="group.title" class="tw-rounded-lg tw-border tw-border-slate-200 tw-bg-white tw-p-4">
            <h3 class="tw-mb-3 tw-text-sm tw-font-bold tw-uppercase tw-text-slate-500">{{ group.title }}</h3>
      ...
      <v-dialog v-model="editDialog" max-width="900">
        <v-card>
          <v-card-title>Edit Enrollee</v-card-title>
          <v-card-text>
...
const printIdCard = async (item) => {
  try {
    ...
    downloadBlob(response.data, `enrollee_id_card_${item.enrollee_id || item.id}.pdf`);
  } catch (e) {
    error(e.response?.data?.message || 'Could not download ID card');
  }
};
